...
function log_connection_packet(text) {
    let date = log_date();
    console.log("%c[" + date + "] [PACKET] > " + text, "color: #1AF8F5; font-weight: bold;")
}

function log_connection_close(text) {
    let date = log_date();
    console.log("%c[" + date + "] [CONNECTION] > " + text, "color: #F81A9A; font-weight: bold;")
}